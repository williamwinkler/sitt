...
impl ProjectService {
    ...
    pub async fn create(&self, project_name: &str, user: &str) -> Result<Project, ProjectError> {
        // Get existing projects for user
        let existing_projects = match self.repository.get_all(user).await {
    ...
    pub async fn get_all(&self, user: &str) -> Result<Vec<Project>, ProjectError> {
        match self.repository.get_all(user).await {
            Ok(mut projects) => {
    ...
    pub async fn get(&self, project_id: &str, user: &str) -> Result<Project, ProjectError> {
        let result = self.repository.get(project_id, user).await;
    ...
    pub async fn update(&self, project: &mut Project, user: &str) -> Result<Project, ProjectError> {
